...
static rt_size_t _read_10(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    max_buff_len = data->count * data->geometry.bytes_per_sector > MSTORAGE_BUFF_MAX_SIZE ?
                   MSTORAGE_BUFF_MAX_SIZE : data->count * data->geometry.bytes_per_sector;
    data->csw_response.data_reside = data->cb_data_size;
    RT_DEBUG_LOG(RT_DEBUG_USB, ("_read_10 count 0x%x block 0x%x cb_data_size=%d\n",
                                data->count, data->block, data->cb_data_size));
    /*For file systems, reading data in one go can improve speed*/
    size = rt_device_read(data->disk, data->block, data->ep_in->buffer, max_buff_len / data->geometry.bytes_per_sector);
    if (size == 0)
    {
    ...
    data->ep_in->request.size = max_buff_len;
    data->ep_in->request.req_type = UIO_REQUEST_WRITE;
    rt_usbd_io_request(func->device, data->ep_in, &data->ep_in->request);
...
static rt_size_t _write_10(ufunction_t func, ustorage_cbw_t cbw)
{
    ...
    RT_DEBUG_LOG(RT_DEBUG_USB, ("_write_10 count 0x%x block 0x%x xfer_len=%d\n",
                                data->count, data->block, cbw->xfer_len));
    data->csw_response.data_reside = data->cb_data_size;

    data->ep_out->request.buffer = data->ep_out->buffer;
    max_buff_len = data->count * data->geometry.bytes_per_sector > MSTORAGE_BUFF_MAX_SIZE ?
                   MSTORAGE_BUFF_MAX_SIZE : data->count * data->geometry.bytes_per_sector;
    /*Read the requested data all at once to increase efficiency*/
    data->ep_out->request.size = max_buff_len;
...
static rt_err_t _ep_out_handler(ufunction_t func, rt_size_t size)
{
    ...
    else if (data->status == STAT_RECEIVE)
    {
        ...
        rt_device_write(data->disk, data->block, data->ep_out->buffer, size /
                        data->geometry.bytes_per_sector);
...
static rt_err_t _function_enable(ufunction_t func)
{
    ...
    data->ep_in->buffer = (rt_uint8_t *)rt_malloc(MSTORAGE_BUFF_MAX_SIZE);
    if (data->ep_in->buffer == RT_NULL)
    {
        rt_kprintf("no memory\n");
        return -RT_ENOMEM;
    }
    data->ep_out->buffer = (rt_uint8_t *)rt_malloc(MSTORAGE_BUFF_MAX_SIZE);
