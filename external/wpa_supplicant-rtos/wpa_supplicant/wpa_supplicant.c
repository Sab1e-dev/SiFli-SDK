...
int wpas_build_ext_capab(struct wpa_supplicant *wpa_s, u8 *buf, size_t buflen)
{
    ...
    {
        wpa_printf(WPA_MSG_LOG, "Not enough room for building extended capabilities element");
        return -1;
    }
...
/**
 * 根据指定的 BSS（扫描结果）和网络配置（struct wpa_ssid），
 * 请求 wpa_supplicant 进行一次关联（association）操作，即尝试连接到指定的 Wi-Fi 网络
                                   ...
                                   enum wpa_drv_update_connect_params_mask *mask)
{
    ...
    {
        wpa_printf(WPA_MSG_LOG, "TESTING: Override OWE DH element");
    }
    else
#endif /* CONFIG_TESTING_OPTIONS */
...
pfs_fail:
    ...
    if (wpa_bss_ext_capab(bss, WLAN_EXT_CAPAB_MSCS) && wpa_s->robust_av.valid_config)
    {
        ...
        if (!mscs_ie)
        {
            wpa_printf(WPA_MSG_LOG, "MSCS: Failed to allocate MSCS IE");
            goto mscs_end;
        }
 ...
 */
/*
根据指定的网络配置（struct wpa_ssid *ssid），尝试与该网络进行关联（连接），
并自动管理所有网络的使能/禁用状态。
*/
...
wpa_printf(WPA_MSG_LOG,"%s %d\n", __func__, __LINE__);
    /*
将所有其他网络标记为禁用，或者在未指定工作的情况下将所有网络标记为启用。
     */
    wpa_printf(WPA_MSG_LOG,"%s %d\n", __func__, __LINE__);
    for (other_ssid = wpa_s->conf->ssid; other_ssid; other_ssid = other_ssid->next)
    {
...
wpa_printf(WPA_MSG_LOG,"%s %d\n", __func__, __LINE__);
    ...
    wpa_printf(WPA_MSG_LOG,"%s %d\n", __func__, __LINE__);
    wpa_supplicant_fast_associate(wpa_s);//关联并连接
    if (0)//(wpa_s->connect_without_scan || wpa_supplicant_fast_associate(wpa_s) != 1)//看是否关联成功，如果没有 那就重新扫描一次
    {
        wpa_s->scan_req = NORMAL_SCAN_REQ;
        wpa_printf(WPA_MSG_LOG,"%s %d\n", __func__, __LINE__);
...
wpa_printf(WPA_MSG_LOG,"%s %d \n",__func__,__LINE__);
    ...
    {
        wpa_printf(WPA_MSG_LOG, "RX EAPOL - ignore_auth_resp active!");
        return;
    }
#endif /* CONFIG_TESTING_OPTIONS */
wpa_printf(WPA_MSG_LOG,"%s %d wpa_s->wpa_state=%d\n",__func__,__LINE__,wpa_s->wpa_state);
...
#endif /* CONFIG_AP */
wpa_printf(WPA_MSG_LOG,"%s %d wpa_s->key_mgmt=0x%x\n",__func__,__LINE__,wpa_s->key_mgmt);
    if (wpa_s->key_mgmt == WPA_KEY_MGMT_NONE)
    {
        wpa_dbg(wpa_s, MSG_DEBUG,
...
wpa_printf(WPA_MSG_LOG,"%s %d \n",__func__,__LINE__);
    ...
    {
        /*
         *如果驱动支持4路握手，且当前为802.1X认证（如企业WIFI）
说明4路握手由驱动完成，supplicant只需完成EAP认证流程。
直接将EAPOL状态机的 portValid 标志设为 true，允许数据流通，跳过EAPOL-Key握手流程
         */
        wpa_printf(WPA_MSG_LOG,"%s %d \n",__func__,__LINE__);
...
wpa_printf(WPA_MSG_LOG,"%s: %d\n", __func__, __LINE__);
        ...
        {
            wpa_printf(WPA_MSG_LOG,"%s: %d\n", __func__, __LINE__);
            wpa_supplicant_req_scan(wpa_s, interface_count % 3, 10000);
        }
...
static int wpas_fst_send_action_cb(void *ctx, const u8 *da, struct wpabuf *data)
{
    ...
    {
        wpa_printf(WPA_MSG_LOG, "FST:%s:bssid=" MACSTR " != da=" MACSTR, __func__, MAC2STR(wpa_s->bssid), MAC2STR(da));
        return -1;
    }
...
next_driver:
    ...
    wpa_printf(WPA_MSG_LOG,"%s %d,ifname=%s\n", __func__, __LINE__,ifname);
...
static int wpa_supplicant_init_iface(struct wpa_supplicant *wpa_s, const struct wpa_interface *iface)
{
    ...
    if (iface->confname)
    {
        ...
        {
            wpa_printf(WPA_MSG_LOG, "Failed to read or parse configuration '%s'.\n", wpa_s->confanother);
            return -1;
        }
    ...
    wpa_printf(WPA_MSG_LOG,"%s %d wpa_s->hw.modes=%p\n", __func__, __LINE__,wpa_s->hw.modes);
    if (wpa_s->hw.modes)
    {
        u16 i;
                                                ...
                                                struct wpa_supplicant *parent)
{
    ...
    wpa_printf(WPA_MSG_LOG,"%s %d,wpa_s=0x%p,iface=%p\n", __func__, __LINE__,wpa_s,iface);
    if (wpa_s == NULL)
        return NULL;
    ...
    if (wpa_supplicant_init_iface(wpa_s, &t_iface))
    {
        ...
        wpa_printf(WPA_MSG_LOG,"%s %d,wpa_s=0x%p,iface->ifname=%p\n", __func__, __LINE__,wpa_s,iface->ifname);
        RT_ASSERT(0);
        wpa_supplicant_deinit_iface(wpa_s, 0, 0);

        return NULL;
    }
    wpa_printf(WPA_MSG_LOG,"%s %d,wpa_s=0x%p\n", __func__, __LINE__,wpa_s);
    if (iface->p2p_mgmt == 0)
    {
    ...
    global->ifaces = wpa_s;
    wpa_printf(WPA_MSG_LOG,"%s %d,global=0x%p\n", __func__, __LINE__,global);
    wpa_dbg(wpa_s, MSG_DEBUG, "Added interface %s", wpa_s->ifname);
    wpa_supplicant_set_state(wpa_s, WPA_DISCONNECTED);
    wpa_printf(WPA_MSG_LOG,"%s %d,global->ifaces=0x%p\n", __func__, __LINE__,global->ifaces);
    ...
    {
        wpa_printf(WPA_MSG_LOG, "P2P: Failed to enable P2P Device interface");
...
void wpas_request_connection(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_printf(WPA_MSG_LOG,"%s %d\n", __func__, __LINE__);
    if (wpa_supplicant_fast_associate(wpa_s) != 1)
        wpa_supplicant_req_scan(wpa_s, 0, 0);//发起一次新的扫描
    else
        wpa_s->reattach = 0;
}
...
int wpas_enable_mac_addr_randomization(struct wpa_supplicant *wpa_s, unsigned int type, const u8 *addr, const u8 *mask)
{
    ...
    {
        wpa_printf(WPA_MSG_LOG, "MAC_ADDR_RAND_SCAN invalid addr/mask combination");
        return -1;
    }

    if (addr && mask && (!(mask[0] & 0x01) || (addr[0] & 0x01)))
    {
        wpa_printf(WPA_MSG_LOG, "MAC_ADDR_RAND_SCAN cannot allow multicast address");
        return -1;
    }
