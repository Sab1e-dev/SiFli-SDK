...
void sme_event_auth(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
{
    ...
    if (data->auth.status_code != WLAN_STATUS_SUCCESS)
    {
        ...
        {
            case WLAN_AUTH_OPEN://无需密码的验证  开放系统认证
                wpa_s->current_ssid->auth_alg = WPA_AUTH_ALG_SHARED;

                wpa_dbg(wpa_s, MSG_DEBUG, "SME: Trying SHARED auth");
                wpa_supplicant_associate(wpa_s, wpa_s->current_bss, wpa_s->current_ssid);//开始（Association）阶段
                return;

            case WLAN_AUTH_SHARED_KEY://共享密钥认证
...
rt_kprintf("%s %d auth_type=%d\n", __func__, __LINE__,data->auth.auth_type);
//进入关联（Association）阶段
    sme_associate(wpa_s, ssid->mode, data->auth.peer, data->auth.auth_type);//开始（Association）阶段
}
...
mscs_fail:
    ...
    if (wpa_s->p2pdev->set_sta_uapsd)
        ...
        params.uapsd = -1;
rt_kprintf("%s %d params=%p\n", __func__, __LINE__,params);//params已经包含了ies信息
