...
SM_STEP(SUPP_PAE)
{
    if ((sm->userLogoff && !sm->logoffSent) && !(sm->initialize || !sm->portEnabled))
        ...
        switch (sm->SUPP_PAE_state)
        {
            ...
            rt_kprintf("%s %d startWhen=%d,startCount=%d,maxStart=%d,portValid=%d,sm->eapSuccess=%d,sm->eapFail=%d\n",__func__,__LINE__
                            ,sm->startWhen,sm->startCount,sm->maxStart,sm->portValid,sm->eapSuccess,sm->eapFail);
